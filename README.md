...
- **Release column** — per-track game-release label (e.g. "A Link to the Past", "Majora's Mask", "Wind Waker"). Computed two ways: subdirectories under a game dir (`docs/midi-files/<game>/<release>/`) take precedence, otherwise a per-game classifier in `tools/build-manifest.js` matches filename patterns. Currently classified: zelda → 5 releases (NES, ALTTP, Link's Awakening, Majora's Mask, Wind Waker). Other multi-title games (Final Fantasy, Donkey Kong Country, Mega Man, Mario, Castlevania) can get classifiers on demand.
